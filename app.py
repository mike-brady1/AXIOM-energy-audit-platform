...
def make_chart(actual_eui, b, activity):
    ...
    ax.set_title("EUI Benchmark Comparison - " + activity,
...
"):
    ...
    story.append(Paragraph(
        "AXIOM Automated Energy Audit Platform | Powered by Claude AI | ISO 50002:2014 | EU EED 2023/1791",
        s_meta))
    doc.build(story)
    return path
...
def run_audit(activity, floor_area, consumption, carrier):
    ...
    benchmark_md += "| **Savings Potential** | " + str(sav_kwh) + " kWh/yr / EUR " + str(sav_eur) + "/yr / " + str(sav_co2) + " kgCO2e/yr |\n"
    ...
    ecm_text = "\n".join([
        "- " + e["ECM"] + ": " + e["kWh/yr"] + " kWh/yr, EUR " + e["EUR/yr"] + "/yr, " + e["Payback"] + " payback"
        for e in ecms
    ])
    prompt = (
        ...
        "Building: " + activity + ", " + str(floor_area) + " m2, EUI " + str(round(actual,1)) +
        " kWh/m2/yr, performance: " + tier + "\n"
        "ECMs:\n" + ecm_text + "\n"
        "Total savings: " + str(total_kwh) + " kWh/yr, EUR " + str(total_eur) + "/yr, " + str(total_co2) + " kgCO2e/yr"
    )
